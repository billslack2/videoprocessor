...
Run scripts/build-vp.ps1 with Checkout, ExpectedCommit, BetaCommit, optional InstallerToolingCommit, CoreVersion, VsInstallPath, QtRoot, VcRedistPath, IsccPath and a new OutputDirectory. Add PortableZip when requested. Installer names include version and actual source commit (12 hex characters), just like the ZIP; dirty test builds additionally retain their source fingerprint. Use the installer identity helper as the filename authority and preserve SHA-256 sidecars. Pass full paths; record chosen tool inputs. It requires Inno Setup 6.7.3, performs the existing full x64 Release installer build, runs all unit tests plus installer support/identity/runtime checks, verifies payload hashes, and exports only after those checks pass. No fabricated build receipts or copying an old build under a new identity. A failed suite stops export; investigate and preserve the failing run before retrying. After investigation, ReuseVerifiedBuild may explicitly resume packaging and validation from the existing successful build receipt; the builder still verifies exact clean source identity and all four binary hashes, and all tests run again. It never automatically retries a failure.
