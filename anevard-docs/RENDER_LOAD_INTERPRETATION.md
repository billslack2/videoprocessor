...
`GPU 10s` shows the average and worst accepted GPU interval in the latest ten
seconds. Its percentage uses the peak, because one over-budget frame can be
visible even when the average is low.
...
started, excluding warm-up. It does not decay with the ten-second window.
...
The GPU number normally arrives a few frames after the frame was submitted.
