...
void CVideoProcessorDlg::UpdateState()
{
	...
	if (m_failedRendererRetirement)
	{
		...
		if (now < m_failedRendererRetirementNextRetryTick)
			return;
		m_failedRendererRetirement->Retire();
		if (!m_failedRendererRetirement->RetirementSucceeded())
		{
			m_failedRendererRetirementNextRetryTick = now + 1000;
			DebugLog::Log(
				"Renderer handoff remains blocked: generation=%u "
				"external-state restoration retry failed",
				m_rendererGeneration.load(std::memory_order_acquire));
			return;
		}
		DebugLog::Log(
			"Renderer handoff restoration retry succeeded: generation=%u",
			m_rendererGeneration.load(std::memory_order_acquire));
		m_failedRendererRetirement.reset();
...
void CVideoProcessorDlg::DestroyVideoRenderer()
{
	...
	if (!m_activeRendererIsDirectShow)
	{
		...
		const bool retirementSucceeded =
			rendererToDestroy->RetirementSucceeded();
		m_rendererTransitionWindow.KeepOnTop();
		...
		{
			m_failedRendererRetirement = std::move(rendererToDestroy);
		...
		DebugLog::Log(
			...
			retirementSucceeded ? "restored" : "pending-retry",
