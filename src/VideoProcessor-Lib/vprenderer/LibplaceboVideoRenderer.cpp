...
struct LibplaceboVideoRenderer::Impl
{
	...
	std::string externalHdrFrameDecisionSignature;
	...
	void ConfigureRenderParams(const RendererSettings& settings,
		const char* lifecycle, bool requestedDisplayLutActive)
	{
		...
		if (!LibplaceboRenderParameters::Build(parameterSettings,
			requestedDisplayLutActive, projection, projectionError))
		{
			throw std::runtime_error(
				"could not configure libplacebo render parameters: " + projectionError);
		}
		if (requestedDisplayLutActive &&
		...
		// Build the complete projection before publishing either half of this
		// transition. A failed rebuild must not leave Active paired with the
		// ordinary no-LUT parameters (or vice versa).
		displayLutActive = requestedDisplayLutActive;
		...
		uint64_t applicationProfileGeneration)
	{
		...
		DebugLog::Log(
			"external HDR 3D LUT generation committed: transaction=%llu resource_generation=%llu mode=%s status=%s activation=frame_gated",
			static_cast<unsigned long long>(transactionGeneration),
			static_cast<unsigned long long>(
	...
	void RejectDisplayLutAfterRenderFailure()
	{
		...
		{
			// Never retry a resource after pl_render rejected it. The previous
			// parameters are conservative (error diffusion remains disabled),
			// and a later successful settings projection restores normal dither.
			displayLutActive = false;
			displayLutParsed = false;
			pl_lut_free(&displayLut);
			throw;
	...
	void SetDisplayLutActive(bool active)
	{
		if (displayLutActive == active)
			return;
		ConfigureRenderParams(activeSettings,
			active ? "LUT activation" : "LUT contract rejection", active);
	}

	const char* DisplayLutContractMismatch(
		const struct pl_frame& target) const
	{
		if (!displayLutParsed || !displayLut)
			return nullptr;
		...
		const auto range = [](enum pl_color_levels value)
		{
			if (value == PL_COLOR_LEVELS_FULL) return InputRange::FULL;
			if (value == PL_COLOR_LEVELS_LIMITED) return InputRange::LIMITED;
		...
		context.carrier.presentationModel =
			actualOutput.presentationModel ==
				LibplaceboOutput::PresentationModel::FLIP ?
				PresentationModel::FLIP :
			(actualOutput.presentationModel ==
				LibplaceboOutput::PresentationModel::BITBLT ?
				PresentationModel::BITBLT : PresentationModel::UNKNOWN);
		context.carrier.vpOwnsPresentation = vpOwnedSwapchain != nullptr;
		SetLastError(ERROR_SUCCESS);
		const LONG_PTR windowStyle =
			GetWindowLongPtr(videoHwnd, GWL_STYLE);
		context.carrier.topLevelPresenter =
			(windowStyle != 0 || GetLastError() == ERROR_SUCCESS) &&
			(windowStyle & WS_CHILD) == 0;
		context.carrier.dxgiColorSpaceTag =
			actualOutput.encoding ==
				LibplaceboOutput::DxgiEncoding::FULL_G22_P709 ?
				DxgiCarrierTag::FULL_G22_P709 : DxgiCarrierTag::OTHER;
		context.carrier.surfaceFormat =
			negotiatedSwapchainFormat == DXGI_FORMAT_R10G10B10A2_UNORM ?
				SurfaceFormat::R10G10B10A2_UNORM : SurfaceFormat::OTHER;
		...
		context.carrier.finalTargetPrimaries =
			primaries(target.color.primaries);
		context.carrier.acceptedOutputTransfer = transfer(
			ResolvedPixelTransfer(actualOutput.encoding,
				actualOutput.targetTransfer));
		context.carrier.finalTargetTransfer = transfer(target.color.transfer);
		...
		VideoConversionOverride videoConversionOverride)
	{
		...
		// It remains render-inert until this frame has both a verified HDR carrier
		// and an authorized frame-local PL_LUT_CONVERSION attachment.
	...
	bool ApplyProfileSettingsLiveLocked(const RendererSettings& settings)
	{
		...
		DebugLog::Log(
			...
			sdrTargetNits,
			sdrBlackNits,
			displayLutActive ? "active" : "disabled",
			reportBt2020ToDisplay ? "requested" : "disabled");
		return true;
	}
		...
		double& presentationTargetLeadMs)
	{
				...
				frameExternalHdrColorMap);
		{
			...
			decision << externalHdrProjection.resolved.transactionGeneration << '|'
				<< externalHdrProjection.resolved.resourceGeneration << '|'
				<< externalHdrCarrierArmed << '|'
			...
			if (decision.str() != externalHdrFrameDecisionSignature)
			{
				...
				const char* const outputDisposition =
					externalHdrProjection.attached ? "HDR10_PQ_BT2020" :
					externalHdrCarrierArmed ? "suppressed_no_present" : "internal_SDR";
				DebugLog::Log(
					"external HDR frame decision: transaction=%llu resource_generation=%llu carrier_armed=%d attached=%d slot=%d preconvert=%d reason=%s output=%s",
					static_cast<unsigned long long>(externalHdrProjection.resolved.transactionGeneration),
					static_cast<unsigned long long>(externalHdrProjection.resolved.resourceGeneration),
					externalHdrCarrierArmed ? 1 : 0,
					externalHdrProjection.attached ? 1 : 0,
					static_cast<int>(externalHdrProjection.resolved.selection.slot),
					externalHdrProjection.resolved.selection.requiresExplicitPrimariesTransform ? 1 : 0,
					externalHdrProjection.resolved.selection.reason,
					outputDisposition);
			}
		}
		...
		if (!rendered && externalHdrProjection.attached &&
			externalHdrLuts.RejectRuntimeGeneration(externalHdrProjection.resolved))
		{
			externalHdrLutStatus =
				"Internal fallback; Cube failed during rendering";
			externalHdrFrameDecisionSignature.clear();
			DebugLog::Log(
				"external HDR 3D LUT generation quarantined after render failure: transaction=%llu resource_generation=%llu fallback=internal_SDR",
				static_cast<unsigned long long>(externalHdrProjection.resolved.transactionGeneration),
				static_cast<unsigned long long>(externalHdrProjection.resolved.resourceGeneration));
		}
		...
		else
		{
			...
			const bool frameConsumed = pl_swapchain_submit_frame(swapchain);
			submitted = rendered && frameConsumed;
			if (submitted)
				pl_swapchain_swap_buffers(swapchain);
		}
		...
		if (rendered && submitted)
		{
			...
			{
				if (externalHdrProjection.attached)
				{
					const char* metadataGamut = "unknown";
					switch (activeSettings.externalHdrOutputMetadataPrimaries)
					{
					case LibplaceboExternalHdrLut::Primaries::BT709:
						metadataGamut = "BT.709"; break;
					case LibplaceboExternalHdrLut::Primaries::P3_D65:
						metadataGamut = "P3-D65"; break;
					case LibplaceboExternalHdrLut::Primaries::BT2020:
						metadataGamut = "BT.2020"; break;
					default: break;
					}
					DebugLog::Log(
						"libplacebo external HDR LUT: input=%s/%s mastering=%.3f..%.1f nits -> HDR10 full-range BT.2020/PQ metadata_gamut=%s metadata_peak=%.1f nits",
						CStringA(ToString(state.eotf)).GetString(),
						CStringA(ToString(state.colorspace)).GetString(),
...
bool LibplaceboVideoRenderer::OnVideoState(VideoStateComPtr& videoState)
{
	...
	{
		DebugLog::Log(
			"libplacebo source transition accepted in place: %s/%s -> %s/%s; output contract will be reevaluated on the render thread",
