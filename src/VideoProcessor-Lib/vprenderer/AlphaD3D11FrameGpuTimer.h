...
public:
	...
	bool Initialize(ID3D11Device* device)
	{
		...
		for (Slot& slot : m_slots)
		{
			if (FAILED(device->CreateQuery(&disjoint, &slot.disjoint)) ||
				FAILED(device->CreateQuery(&timestamp, &slot.start)) ||
				FAILED(device->CreateQuery(&timestamp, &slot.end)))
			{
				ReleaseResources();
				return false;
			}
	...
	// End immediately after pl_render_image, before Present or a CPU vsync wait.
	...
	ResolveResult TryResolveOldest(ID3D11DeviceContext* context, Sample& sample)
	{
		...
		result = context->GetData(slot.start, &start, sizeof(start), flags);
		if (result == S_FALSE)
		{
			m_notReady.fetch_add(1, std::memory_order_relaxed);
			return ResolveResult::NotReady;
		...
		if (result != S_OK)
			return DiscardOldest(m_queryFailures);
		result = context->GetData(slot.end, &end, sizeof(end), flags);
		if (result == S_FALSE)
		{
			m_notReady.fetch_add(1, std::memory_order_relaxed);
			return ResolveResult::NotReady;
		...
		const double milliseconds = static_cast<double>(end - start) * 1000.0 /
