...
passed 9/9. A clean GUI rebuild corrected a stale project-output deployment; redeployed the matched x64 Release pair to `C:\Videoprocessor\vp` on 2026-08-31; the prior EXE/DLL pair is preserved at `backup-before-vp0088-clean-gui-20260831-172722`. Deployed hashes match the build (`VideoProcessor.exe` `6079CF6E...6CA5D`, `VideoProcessorVPRenderer.dll` `139FF312...60098`); active `VideoProcessor.cfg` was not modified (SHA-256 `E1B76448...6C7A9`). Live flip-model validation remains. On 2026-08-31 the deployed runtime was intentionally returned to the clean official `v1.3.004-beta` baseline at `e9ed97c2`; the corresponding source branch remains pushed for later integration. The prior installed EXE/DLL are backed up at `backup-before-v1.3.004-beta-e9ed97c2-20260831-201633`; the baseline deployment hash-verifies (`VideoProcessor.exe` `92132763...FA270`, `VideoProcessorVPRenderer.dll` `5E5609C4...18B97`) and leaves active `VideoProcessor.cfg` unchanged. Rebuilt and redeployed the same rebased commit on 2026-08-31 after confirming `origin/v1.3.004-beta` remained at `e9ed97c2`; prior deployed binaries are preserved at `backup-before-vp0088-rebuild-20260831-174154`. The replacement pair hash-verifies as `VideoProcessor.exe` `AA014D0E...C79DD` and `VideoProcessorVPRenderer.dll` `FD650F57...553C8`; active `VideoProcessor.cfg` remained unchanged (SHA-256 `E1B76448...6C7A9`).
